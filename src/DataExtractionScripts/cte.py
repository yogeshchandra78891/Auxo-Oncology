...
def _parse_study(study: dict) -> dict:
    ...
    return {
        ...
        "interventions": ", ".join(
            item.get("name", "") for item in arms_module.get("interventions", [])
        ),
...
def fetch_top_10_clinical_trials(
    ...
    try:
        ...
        print(f"[ERROR] ClinicalTrials.gov request failed: {exc}")
    ...
    if not studies:
        ...
        print(f"[WARN] No studies found for: {keyword}")
    ...
    for study in studies:
        ...
        if (
            not record.get("summary", "").strip()
            and not record.get("detailed_description", "").strip()
        ):

            print(
                f"[skip] {nct_id} has no summary or description — trying next candidate"
            )
    ...
    for index, record in enumerate(records, start=1):
        ...
        print(f"{index:2}. " f"{record['nct_id']} | " f"{record['title']}")
    ...
    print(f"\n[cte] Saved " f"{len(records)} studies → {output}")
